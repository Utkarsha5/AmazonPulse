 ...
 const isCartPage = window.location.href.includes("cart/view") || window.location.href.includes("tez/browse/cart");
  ...
  function overrideAmazonCart() {
    ...
    chrome.storage.local.get("pulseSyncCart", (result) => {
      ...
      // If Pulse cart is empty, don't override anything
      if (!initialCart || !initialCart.items || initialCart.items.length === 0) return;

      // 2. Hide only the "Empty Basket" message, not the container
      const suppressStyle = document.createElement("style");
      suppressStyle.id = "amazon-pulse-cart-suppress";
      suppressStyle.textContent = `
        #scrollableMainBody > div:has(> img[alt="Empty Basket"]) {
          display: none !important;
        }
        #scrollableMainBody > div:has(> button:only-child) {
          display: none !important;
        }
        #pulse-cart-override {
      ...
      wrapper.style.cssText = "width: 100%; position: relative; padding: 16px; box-sizing: border-box; background: #fff; z-index: 10;";
      ...
      function renderPulseCart(cart) {
        ...
        wrapper.innerHTML = `
          <div class="pulse-cart-container" style="border: 2px solid #00a8a8; border-radius: 12px; background: #fff; overflow: hidden; margin-bottom: 20px; box-shadow: 0 4px 16px rgba(0,168,168,0.12);">
            <div class="pulse-cart-header" style="background: linear-gradient(135deg, #f0fcfc 0%, #e6f9f9 100%); padding: 16px; border-bottom: 1px solid #e0e0e0;">
              <span class="pulse-cart-header__icon" style="font-size: 24px; margin-right: 12px;">⚡</span>
              <div style="display: inline-block; vertical-align: top;">
                <h1 class="pulse-cart-header__title" style="margin: 0; font-size: 18px; color: #0f1111;">Amazon Pulse — Intent Cart</h1>
                <p class="pulse-cart-header__sub" style="margin: 4px 0 0; font-size: 12px; color: #565959;">
                  Bundle: <strong style="color:#00a8a8;">${escHtml(cart.bundle_name || "My Pulse Cart")}</strong>
      ...
      const tryInjectCart = setInterval(() => {
        ...
        if (targetContainer) {
          ...
          clearInterval(tryInjectCart);
          // Fallback: inject after Amazon Now header or at top of body
  ...
  function showOrderConfirmation(cart) {
    ...
    screen.innerHTML = `
      <div class="pulse-confirm__card">
        ...
        <h1 class="pulse-confirm__title">🎉 Order Placed!</h1>

        <p class="pulse-confirm__sub">
          Your intent-matched bundle has been confirmed.<br>
          Estimated delivery: <strong>8–10 minutes</strong>
        </p>
  ...
  function injectConfidenceBadge(card) {
    ...
    badge.textContent = "⚡ Pulse Match 94%";
 ...
 function enhanceProductCard(card) {
  ...
  function observeProductCards() {
    ...
    const observer = new MutationObserver(() => {
      ...
      debounceTimer = setTimeout(enhanceAllProductCards, 200);
    });
    ...
    // Also run periodically to catch lazy-loaded cards
    setInterval(enhanceAllProductCards, 2000);
  }
  ...
  function setupFrictionlessCard() {
    ...
    btn.addEventListener("click", () => {
      ...
      btn.disabled = true;
      btn.innerHTML = `<span style="font-size: 16px;">⚙️</span> Adding to Cart...`;
      btn.style.background = "linear-gradient(135deg, #d97706 0%, #b45309 100%)";
      btn.style.boxShadow = "0 4px 10px rgba(217, 119, 6, 0.3)";

      // Save to Pulse cart
      saveToPulseCart({
        bundle_name: "Zero-Friction Restock",
        items: [ITEM_NAME],
        total_price: ITEM_PRICE,
      });

      // Call backend for tracking (non-blocking)
      chrome.runtime.sendMessage({
        type: "FRICTIONLESS_BUY",
        payload: { title: ITEM_NAME, price: String(ITEM_PRICE), user_id: "hackathon_demo_user" }
      }, () => {
        if (chrome.runtime.lastError) console.log("[Pulse]", chrome.runtime.lastError.message);
      });

      // Show success and redirect to cart
      setTimeout(() => {
        btn.innerHTML = `✅ Added to Pulse Cart!`;
          ...
          <div class="amazon-pulse-sidebar__module-content">
            ...
            <div class="amazon-pulse-intent-form">
              ...
              <input
                ...
                placeholder="Try: Bake a cake, Movie night, Gym..."
  ...
  function showContextPopup(popupData) {
    ...
    buyBtns.forEach(btn => {
      btn.addEventListener("click", (e) => {
        ...
        // Show success then redirect to cart
        setTimeout(() => {
          btn.innerHTML = `✅ Added!`;
          btn.style.background = "linear-gradient(135deg, #059669 0%, #047857 100%)";
  ...
  window.addEventListener("keydown", (event) => {
    if (event.altKey && event.code === "KeyR") {
      ...
      showContextPopup({
        message: "🌧️ It's raining! Pick your comfort bundle:",
        bundles: [
          {
            bundle_name: "☕ Chai & Pakoda Vibes",
            items: ["Tata Sampann Pakoda Mix", "Brooke Bond Red Label Tea", "Haldiram's Bhujia"],
            price_inr: 285
          },
          {
            bundle_name: "🍜 Spicy Ramen & Chill",
            items: ["Samyang Buldak Ramen (2 Pack)", "Coke Zero 300ml", "Lay's India's Magic Masala"],
            price_inr: 340
          },
          {
  ...
  setInterval(() => {
    if (window.location.href !== lastUrl) {
      ...
      if (nowOnCart) {
        // Navigated TO cart page — remove sidebar, show Pulse cart
        ...
        // On a browse/category page — clean up cart UI, ensure sidebar exists
        const pulseCart = document.getElementById("pulse-cart-override");
        if (pulseCart) pulseCart.remove();
        ...
        if (hiddenStyles) hiddenStyles.remove();

        // Restore sidebar if not present
        if (!document.getElementById(SIDEBAR_ID)) {
          injectPulseSidebar();
          loadStockoutAlert();
        }
